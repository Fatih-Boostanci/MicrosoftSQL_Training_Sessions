...
SELECT ss.staff_id, ss.first_name, ss.last_name, ps.product_id, ps.quantity,
        MAX(ps.quantity) 
        MAX(PARTITION BY ps.product_id) as quantity_product
FROM sale.staff ss, product.stock ps 
WHERE ss.store_id = ps.store_id
---
SELECT ss.staff_id, ss.first_name, ss.last_name, ps.product_id, ps.quantity,
        MIN(ps.quantity) 
        OVER (PARTITION BY ps.product_id) as quantity_product
FROM sale.staff ss, product.stock ps 
WHERE ss.store_id = ps.store_id 

---
SELECT ss.staff_id, ss.first_name, ss.last_name, ps.product_id, 
        AVG(ps.quantity) as average, SUM(ps.quantity) as total
FROM sale.staff ss, product.stock ps 
WHERE ss.store_id = ps.store_id 
GROUP BY ss.staff_id, ss.first_name, ss.last_name, ps.product_id 
-----
